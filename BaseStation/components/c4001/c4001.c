...
#define C4001_I2C_ADDR    0x2A
#define C4001_I2C_FREQ_HZ 50000


static esp_err_t write_reg_i2c(c4001_t *sensor, uint8_t reg, uint8_t *data, uint8_t len) {
    uint8_t buf[len + 1];
    buf[0] = reg;
...
static esp_err_t read_reg_i2c(c4001_t *sensor, uint8_t reg, uint8_t *data, uint8_t len) {
    if (len == 0) {
        return ESP_OK;
    }
...
esp_err_t c4001_init(c4001_t *sensor, i2c_master_bus_handle_t bus_handle) {
    i2c_device_config_t dev_cfg = {
        ...
        .device_address  = C4001_I2C_ADDR,
        .scl_speed_hz    = C4001_I2C_FREQ_HZ,
    };
...
void c4001_set_sensor(c4001_t *sensor, set_cmd_t cmd) {
    uint8_t temp = cmd;
...
esp_err_t c4001_set_mode(c4001_t *sensor, sensor_mode_t mode) {
    sensor_status_t data = c4001_get_status(sensor);

    if (data.work_mode == mode) {
        return ESP_OK;
    }

    c4001_set_sensor(sensor, CHANGE_MODE);
    data = c4001_get_status(sensor);
...
esp_err_t c4001_set_trig_sensitivity(c4001_t *sensor, uint8_t sensitivity)
{
    ...
    c4001_set_sensor(sensor, SAVE_PARAMS);
    return ESP_OK;
}
