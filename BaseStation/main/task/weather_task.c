...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_DATA:
            ...
            // Use static buffer - no malloc/realloc needed
            size_t new_len = response->buffer_len + evt->data_len + 1;
            if (new_len > 52428) {  // 51 KB buffer limit
                ESP_LOGE(TAG, "Response buffer too large, aborting");
...
void weather_task(void *pvParameter) {
    ...
    // Static buffer to prevent memory fragmentation over time
    static char response_buffer[52428];  // 51 KB buffer
    ...
    for (;;) {
        ...
        // Ensure cleanup happens even if we continue early
        if (client == NULL) {
            ESP_LOGE(TAG, "Failed to initialize HTTP client");
            vTaskDelay(pdMS_TO_TICKS(1000 * 60 * 15));
            continue;
        }
        ...
        if (err == ESP_OK) {
            ...
            if (json == NULL) {
                ...
                if (current == NULL || !cJSON_IsObject(current)) {
                    ...
                    cJSON_Delete(json);
                    if (response.buffer) {
                        response.buffer_len = 0;  // Reset buffer length instead of freeing
                    }
        ...
        // Clean up HTTP client at the end of each loop iteration
        esp_http_client_cleanup(client);
